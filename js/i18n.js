/* ============================================
   PDflow — i18n.js
   中英文切换 · 字典 + 替换逻辑
   ============================================ */
(function(){
'use strict';

var DICTS={
en:{
/* Meta */
'page.title':'PDflow — Visual Document Builder & PDF Conversion | Yinliu',
'page.desc':'Create documents visually. Convert PDFs with OCR. Export instantly. Local-first document platform for designers and teams.',
'og.title':'PDflow — Visual Document Builder & PDF Conversion',
'og.desc':'Create documents visually. Convert PDFs with OCR. Export instantly. Local-first.',

/* Nav */
'nav.product':'Product',
'nav.compare':'Compare',
'nav.cases':'Cases',
'nav.faq':'FAQ',
'nav.blog':'Blog',
'nav.download':'Download Free',
'lang.toggle':'中',

/* Hero */
'hero.badge':'v1.2 Now Available',
'hero.h1':'Create Documents<br>Like Designing.',
'hero.sub':'Visual document builder with PDF conversion, OCR, and professional export. No cloud. No signup.',
'hero.cta.primary':'Download Free',
'hero.cta.secondary':'See Product',
'hero.trust':'★★★★★ <span>Free forever · 10,000+ documents generated</span>',
'hero.float1':'PDF → Excel',
'hero.float2':'OCR Ready',
'hero.float3':'Export PDF',

/* S2 Demo */
's2.tag':'Product',
's2.h2':'See PDflow in Action',
's2.desc':'From import to export. Real-time preview, professional output.',
's2.tab1':'Invoice Template',
's2.tab2':'PDF → Excel',
's2.f1.num':'6+','s2.f1.t':'Templates','s2.f1.s':'Invoices, contracts, reports',
's2.f2.num':'10+','s2.f2.t':'PDF Tools','s2.f2.s':'Merge, split, compress, OCR',
's2.f3.num':'100%','s2.f3.t':'Local','s2.f3.s':'No cloud, no upload',
's2.f4.num':'<3s','s2.f4.t':'Export','s2.f4.s':'High-quality PDF',

/* S3 Gallery */
's3.tag':'Gallery',
's3.h2':'Built for real work',
's3.desc':'Professional template layout, PDF conversion, and full toolbox — all in one desktop app.',
's3.c1.t':'Template Editor','s3.c1.s':'Visual layout with live preview',
's3.c2.t':'PDF Conversion','s3.c2.s':'PDF → Excel with OCR',
's3.c3.t':'PDF Toolbox','s3.c3.s':'Merge, split, compress, watermark',

/* S4 Compare */
's4.tag':'Benchmark',
's4.h2':'Built differently',
's4.desc':'See how PDflow compares to online PDF tools.',
's4.th1':'Feature','s4.th2':'PDflow','s4.th3':'Online PDF Tools',
's4.r1':'100% Local Processing',
's4.r2':'No File Upload',
's4.r3':'OCR Conversion',
's4.r4':'Professional Templates',
's4.r5':'PDF Export Quality',
's4.r5.p1':'Vector','s4.r5.p2':'Raster',
's4.r6':'Free (No Subscription)',
's4.r7':'Works Offline',
's4.r8':'Privacy Guaranteed',
's4.note':'* Online tools often limit OCR to first 5 pages unless paid.',

/* S5 Cases */
's5.tag':'Real Cases',
's5.h2':'From busy to done',
's5.desc':'Real teams. Real documents. Real time saved.',
's5.c1.b':'Freelancer',
's5.c1.b1':'Before','s5.c1.b1t':'30 min manual layout',
's5.c1.a1':'After','s5.c1.a1t':'5 min — export done',
's5.c1.q':'"I generate invoices in minutes. Template system is a lifesaver."',
's5.c2.b':'Accounting',
's5.c2.b1':'Before','s5.c2.b1t':'Manual PDF → Excel typing',
's5.c2.a1':'After','s5.c2.a1t':'OCR auto-extracts tables',
's5.c2.q':'"99% accuracy on scanned invoices. No retyping needed."',
's5.c3.b':'Operations',
's5.c3.b1':'Before','s5.c3.b1t':'Multiple tools for each PDF task',
's5.c3.a1':'After','s5.c3.a1t':'One app for merge, split, OCR',
's5.c3.q':'"Merge, compress, OCR — all local. Our data never leaves."',
's5.c4.b':'Agency',
's5.c4.b1':'Before','s5.c4.b1t':'Client docs need reformatting',
's5.c4.a1':'After','s4.a1t':'Template + brand style = done',
's5.c4.a1t':'Template + brand style = done',
's5.c4.q':'"Brand templates with consistent formatting — clients love it."',

/* S6 FAQ */
's6.tag':'FAQ',
's6.h2':'Frequently asked<br>questions',
's6.search':'Search questions...',
's6.faq.placeholder':'',
's6.q1':'Is PDflow really free?',
's6.a1':'Yes. All features included. No hidden fees, no premium tiers, no ads. Built for the community.',
's6.q2':'What platforms are supported?',
's6.a2':'Windows 10/11 (64-bit). macOS and Linux on roadmap. Built with PySide6 Qt 6.',
's6.q3':'Is my data processed locally?',
's6.a3':'100%. All processing on your machine. No cloud upload, no tracking. Works fully offline.',
's6.q4':'How does PDF → Excel conversion work?',
's6.a4':'PyMuPDF + RapidOCR for scanned documents. Table structure preserved. Columns aligned globally. Images embedded directly into Excel cells.',
's6.q5':"What's the file size and version?",
's6.a5':'Current version is v1.2.0. About 78 MB installed. Works on Windows 10 and 11. SHA256 checksum available on the download page.',
's6.q6':'Can I use it commercially?',
's6.a6':'Absolutely. Generate invoices, contracts, and reports for your business or clients with no restrictions. Free for commercial use.',

/* S7 Download */
's7.h2':'Start Creating Better<br>Documents',
's7.p':'Free download. Local-first. No signup required.',
's7.cta.primary':'Download Free',
's7.cta.github':'GitHub Source',
's7.cta.changelog':'View Changelog →',
's7.i.version':'Version','s7.i.updated':'Updated','s7.i.platform':'Platform','s7.i.size':'Size','s7.i.sha':'SHA256',
's7.i.version.v':'1.2.0',
's7.i.updated.v':'Jun 2026',
's7.i.platform.v':'Windows 10/11',
's7.i.size.v':'215 MB',
's7.users':'Downloaded by <strong>1,200+</strong> users · Early Access',

/* Footer */
'f.tag':'Document Creation Platform.<br>Build. Convert. Export. All local.',
'f.h.download':'Download',
'f.d.windows':'Windows','f.d.notes':'Release Notes','f.d.changelog':'Changelog','f.d.sha':'SHA256 Checksums',
'f.h.product':'Product',
'f.p.excel':'PDF → Excel','f.p.tools':'PDF Tools','f.p.template':'Template Builder','f.p.demo':'Demo','f.p.compare':'Compare',
'f.h.resources':'Resources',
'f.r.blog':'Blog','f.r.faq':'FAQ','f.r.docs':'Documentation','f.r.api':'API','f.r.roadmap':'Roadmap',
'f.h.company':'Company',
'f.c.about':'About','f.c.privacy':'Privacy Policy','f.c.terms':'Terms of Service','f.c.support':'Support','f.c.github':'GitHub',
'f.bot.left':'&copy; 2026 YinliuPDflow. <a href="#">Privacy</a> · <a href="#">Terms</a> · <a href="mailto:support@yinliupdflow.com">Contact</a>',
'f.bot.right':'Built with PySide6 · RapidOCR · PyMuPDF · Last updated Jun 2026',

/* Blog index */
'blog.tag':'Blog',
'blog.h1':'PDF tips, guides<br>and product news',
'blog.sub':'Learn how to get the most out of PDflow — from PDF conversion to template design and document automation.',
'blog.read':'Read Article →',
'blog.cta':'Want to see PDflow in action?',
'blog.cta.btn':'Download Free',
'blog.back':'← Back to Blog',

/* Blog detail: v1.2 release */
'v12.breadcrumb':'Blog / Product Releases',
'v12.h1':'PDflow v1.2: A Faster,<br>Cleaner, Smarter Update',
'v12.lede':'A new template library, a 50% faster PDF export engine, a fully redesigned home UI, and a more reliable OCR engine. Everything stays local, free, and unlimited.',
'v12.s1.t':'New Template Library',
'v12.s1.p':'We rebuilt the template system from scratch. The new library ships with 6+ professionally designed templates for invoices, contracts, reports, announcements, and product specs — all built with the unified PDflow layout engine. Each template supports live preview, brand colors, and JSON-based customization, so you can tune every detail without rewriting from zero.',
'v12.s2.t':'50% Faster PDF Export',
'v12.s2.p':'The export pipeline was rewritten around PyMuPDF\'s vector primitives. Large documents that used to take 6 seconds now export in under 3 — a 50% speedup on real-world reports with embedded images, tables, and CJK fonts. Memory usage during export was reduced by 30% as well, so longer documents stay responsive on modest hardware.',
'v12.s3.t':'Refreshed Home UI',
'v12.s3.p':'The desktop homepage, the toolbox launcher, and the settings panel got a full visual pass. New typography scale, refined card spacing, and a redesigned navigation drawer make the app feel more cohesive. The theme system is unified through a single ThemeManager — no more drift between windows.',
'v12.s4.t':'Polished OCR Engine',
'v12.s4.p':'RapidOCR now loads with a smarter preprocessing pass for deskew, denoise, and contrast normalization. Scanned invoices and receipts see 8–15% accuracy gains on small or low-DPI text. Multi-language CJK support is now on by default, with no extra model downloads.',
'v12.s5.t':'Bug Fixes & Stability',
'v12.s5.p':'PDF merge now correctly preserves page rotation flags. Watermark preview no longer flickers on resize. Compress handles mixed-orientation pages without crashing. The template editor\'s undo history is now bounded to 100 steps, preventing runaway memory on long sessions.',
'v12.s6.t':'Upgrade & Feedback',
'v12.s6.p':'v1.2 is a free, in-place upgrade — just install over v1.1. Settings, drafts, and your library are preserved. If you hit anything odd, please open an issue on GitHub or email us. v1.3 will focus on a visual layout canvas with snap guides and zoom controls.',
'v12.kw':'Release notes · v1.2.0 · June 23 2026 · 5 min read',
'v12.cta':'Download v1.2 Free',
'blog.tag.product':'Product','blog.tag.conv':'Conversion','blog.tag.priv':'Privacy','blog.tag.tpl':'Templates','blog.tag.tb':'Toolbox','blog.tag.ocr':'OCR','blog.tag.wf':'Workflow','blog.tag.cmp':'Comparison',
'blog.v12.title':'PDflow v1.2: What\'s New','blog.v12.meta':'Jun 23, 2026 · 5 min read','blog.v12.desc':'Brand-new template library, 50% faster PDF export, refreshed UI with a redesigned homepage, and a polished OCR engine. Everything stays local — no cloud, no signup, still 100% free.',
'blog.bp.title':'Best PDF to Excel Converter in 2026','blog.bp.meta':'Jun 12, 2026 · 6 min read','blog.bp.desc':'Comparing the top PDF-to-Excel tools. Why local OCR processing delivers better accuracy than cloud-based alternatives.',
'blog.ocr.title':'How to Convert PDF to Excel with OCR','blog.ocr.meta':'Jun 8, 2026 · 5 min read','blog.ocr.desc':'Step-by-step guide to converting scanned PDFs into editable Excel spreadsheets with OCR.',
'blog.priv.title':'Why Local PDF Tools Matter for Privacy','blog.priv.meta':'Jun 5, 2026 · 4 min read','blog.priv.desc':'Most online PDF tools upload your documents to third-party servers. Learn why local-first is the only way to guarantee privacy.',
'blog.free.title':'Document Automation for Freelancers','blog.free.meta':'Jun 1, 2026 · 7 min read','blog.free.desc':'How freelancers can save hours each week with automated invoice templates and professional PDF export.',
'blog.inv.title':'How to Design Professional Invoice Templates','blog.inv.meta':'May 28, 2026 · 6 min read','blog.inv.desc':'Design tips for creating invoices that impress clients. CJK typography, table layout, brand formatting.',
'blog.tb.title':'Complete Guide to PDF Tools: Merge, Split, Compress','blog.tb.meta':'May 24, 2026 · 8 min read','blog.tb.desc':'Everything about managing PDFs: merging, splitting, compressing, watermarking.',
'blog.rt.title':'RapidOCR vs Tesseract: Which OCR Engine Is Better?','blog.rt.meta':'May 20, 2026 · 5 min read','blog.rt.desc':'A comparison of OCR engines. Speed, accuracy, CJK support, and why RapidOCR wins for modern workflows.',
'blog.wf.title':'How to Build a Local Document Workflow','blog.wf.meta':'May 16, 2026 · 6 min read','blog.wf.desc':'From template creation to PDF export — design a workflow that keeps your data local.',
'blog.p2i.title':'PDF to Image: When and How to Convert','blog.p2i.meta':'May 12, 2026 · 4 min read','blog.p2i.desc':'Use cases for converting PDF pages to images. High-res export for presentations and print-ready assets.',
'blog.v11.title':'PDflow v1.1: What\'s New (Archive)','blog.v11.meta':'May 8, 2026 · 3 min read','blog.v11.desc':'Release notes for PDflow v1.1. Improved OCR, new templates, faster export. Superseded by v1.2.',
'blog.why.title':'Why We Built PDflow: A Local-First Document Platform','blog.why.meta':'May 2, 2026 · 5 min read','blog.why.desc':'The story behind PDflow. Why local processing and privacy-first design matter more than ever.',
'blog.vsp.title':'PDflow vs PDFgear: Which Tool Fits Your Workflow?','blog.vsp.meta':'Apr 28, 2026 · 7 min read','blog.vsp.desc':'Detailed comparison. Features, privacy, templates, OCR quality, and which tool is right for you.',

/* Subpage: PDF → Excel */
'pe.tag':'PDF → Excel',
'pe.h1':'Convert PDF to Excel<br>with OCR.',
'pe.sub':'Preserve table structure. Local-first processing. 99% accuracy on scanned documents. No upload, no signup.',
'pe.cta.primary':'Download Free',
'pe.cta.secondary':'Learn More',
'pe.f1.t':'Table Structure Preserved',
'pe.f1.s':'Columns, rows, and merged cells stay aligned. Edit in Excel right after export.',
'pe.f2.t':'OCR for Scanned PDFs',
'pe.f2.s':'RapidOCR engine handles scanned invoices, receipts, and contracts with 99% accuracy.',
'pe.f3.t':'100% Local Processing',
'pe.f3.s':'Your documents never leave your machine. No cloud upload, no tracking, full privacy.',
'pe.cta.h2':'Start converting PDFs to Excel<br>in seconds.',
'pe.cta.p':'Free download. Local-first. No signup required.',
'pe.cta.meta':'v1.2.0 · Windows 10/11 · 215 MB',

/* Subpage: PDF Tools */
'pt.tag':'PDF Tools',
'pt.h1':'Merge, split, compress.<br>All in one app.',
'pt.sub':'10+ PDF tools. 100% local. No upload, no signup. Professional-grade output for designers and teams.',
'pt.cta.primary':'Download Free',
'pt.cta.secondary':'Learn More',
'pt.f1.t':'Merge & Split',
'pt.f1.s':'Combine multiple PDFs into one, or split a PDF by page ranges. Batch operations supported.',
'pt.f2.t':'Compress',
'pt.f2.s':'Reduce PDF file size by up to 80% while keeping text vector-based and readable.',
'pt.f3.t':'Watermark',
'pt.f3.s':'Add text or image watermarks. Control position, opacity, rotation, and pages.',
'pt.f4.t':'Page Management',
'pt.f4.s':'Reorder, rotate, delete, or insert pages. Preview thumbnails before saving.',
'pt.f5.t':'Format Convert',
'pt.f5.s':'PDF ↔ images, PDF → Word, PDF → Excel, PDF → PPT. Both directions supported.',
'pt.f6.t':'Privacy First',
'pt.f6.s':'All operations on your machine. Documents never leave. Works fully offline.',
'pt.cta.h2':'10+ PDF tools.<br>One app. All local.',
'pt.cta.p':'Free download. Local-first. No signup required.',
'pt.cta.meta':'v1.2.0 · Windows 10/11 · 215 MB',

/* Subpage: Template Builder */
'tb.tag':'Template Builder',
'tb.h1':'Create documents<br>like designing.',
'tb.sub':'Visual template layout for invoices, contracts, and reports. Live preview. One-click PDF export. 100% local.',
'tb.cta.primary':'Download Free',
'tb.cta.secondary':'Learn More',
'tb.f1.t':'Visual Editor',
'tb.f1.s':'Drag-and-drop layout. Snap to grid. Real-time preview. No coding required.',
'tb.f2.t':'6+ Built-in Templates',
'tb.f2.s':'Invoices, contracts, reports, announcements, business cards, and product specs.',
'tb.f3.t':'Brand Style',
'tb.f3.s':'Save brand colors, fonts, and logos. Reuse across all templates for consistent output.',
'tb.f4.t':'Save Drafts',
'tb.f4.s':'Auto-save your work. Reopen any document. Edit later without losing changes.',
'tb.f5.t':'Vector PDF Export',
'tb.f5.s':'High-quality vector PDF output. Crisp text, accurate layout, professional delivery.',
'tb.f6.t':'JSON Templates',
'tb.f6.s':'Designer-friendly JSON format. Edit by hand or via UI. Share with team.',
'tb.cta.h2':'Start creating beautiful<br>documents today.',
'tb.cta.p':'Free download. Local-first. No signup required.',
'tb.cta.meta':'v1.2.0 · Windows 10/11 · 215 MB',

/* Subpage: About */
'ab.tag':'About',
'ab.h1':'Built for designers.<br>Powered by local-first.',
'ab.sub':'PDflow is a free, open-source document platform that respects your privacy. Every operation runs on your machine — no cloud, no tracking, no signup.',
'ab.cta.primary':'Download Free',
'ab.cta.secondary':'View on GitHub',
'ab.c1.t':'Our Mission',
'ab.c1.p':'Most PDF tools are bloated, expensive, and upload your sensitive documents to the cloud. We built PDflow to fix that. Local-first means your data never leaves your machine — and you don\'t need an internet connection to use the app.',
'ab.c2.t':'Built With',
'ab.c2.p':'A modern stack of open-source foundations. Fast, private, and crafted for designers and teams who care about quality output.',
'ab.c3.t':'Open Source',
'ab.c3.p':'PDflow is open source under the MIT license. View the source on GitHub, submit issues, or contribute. The community decides what gets built next.',
'ab.c4.t':'Roadmap',
'ab.c4.p':'v1.2 (current) — 6+ templates and OCR polishing. v1.3 — visual layout canvas with snap guides. v2.0 — PDflow Studio: unified creation, layout, and AI-assisted export.',
'ab.cta.h2':'Try PDflow today.',
'ab.cta.p':'Free download. Local-first. No signup required.',
'ab.cta.meta':'v1.2.0 · Windows 10/11 · 215 MB · MIT License',
/* Subpage: Privacy Policy */
'pv.tag':'Privacy Policy',
'pv.h1':'Your documents stay<br>on your device.',
'pv.sub':'PDflow is local-first. Your files never touch our servers, because we don\'t have any. Below is the full privacy policy in plain language.',
'pv.updated':'Last updated: June 23, 2026',
'pv.s1.t':'1. Information We Don\'t Collect',
'pv.s1.p':'PDflow runs entirely on your computer. We do not operate cloud servers, do not collect usage telemetry, do not log your files, and do not require an account. The only data the app stores is what you explicitly save to your local disk.',
'pv.s2.t':'2. Analytics on the Website',
'pv.s2.p':'This website (yinliupdflow.com) uses Umami Cloud, a privacy-friendly analytics service. Umami records anonymous page views, button clicks, and aggregate navigation patterns. It does not track you across other websites, does not use cookies for advertising, and does not store IP addresses. The data is used solely to understand which parts of our site are useful.',
'pv.s3.t':'3. Download Links',
'pv.s3.p':'When you download PDflow, your browser may pass the standard HTTP referrer and user-agent to GitHub\'s release server. We do not receive this information.',
'pv.s4.t':'4. Updates & Crash Reports',
'pv.s4.p':'The app can check for updates over HTTPS. It sends your current version number and platform to GitHub Releases; nothing else. We do not collect crash reports, error logs, or stack traces automatically. If you encounter a bug, please report it manually via the GitHub issue tracker.',
'pv.s5.t':'5. Your Rights',
'pv.s5.p':'Because we don\'t collect your data, there is nothing to access, export, or delete. If you contact us by email, we keep the message until you ask us to delete it, and we never share it with third parties.',
'pv.s6.t':'6. Children',
'pv.s6.p':'PDflow is not directed at children under 13. We do not knowingly collect any information from children. The app is a tool, not a service — there is no signup and no profile.',
'pv.s7.t':'7. Changes to This Policy',
'pv.s7.p':'We may update this policy as the product evolves. Material changes will be announced on the blog and noted by a new "Last updated" date. Continued use after a change indicates acceptance.',
'pv.s8.t':'8. Contact',
'pv.s8.p':'Questions? Email support@yinliupdflow.com or open an issue on GitHub. We respond within 2 business days.',
'pv.cta.h2':'Privacy should be the default.',
'pv.cta.p':'Read more in our architecture notes, or download and inspect the app yourself.',
'pv.cta.meta':'v1.2.0 · Local-first · MIT License',
/* Subpage: Terms of Service */
'ts.tag':'Terms of Service',
'ts.h1':'Use PDflow freely.<br>No strings attached.',
'ts.sub':'PDflow is free, open-source, and provided as-is. By using it, you agree to the terms below — written in plain English, no legalese.',
'ts.updated':'Last updated: June 23, 2026',
'ts.s1.t':'1. License',
'ts.s1.p':'PDflow is released under the MIT License. You can use it for personal, commercial, or educational purposes. You can modify it, redistribute it, and sell products built on it. The only requirement is that the copyright notice and license text travel with any redistribution.',
'ts.s2.t':'2. No Warranty',
'ts.s2.p':'PDflow is provided "as is", without warranty of any kind. We test the app thoroughly on Windows 10/11 but cannot guarantee it will work flawlessly on every hardware configuration, in every language, or in every document workflow. Use at your own discretion.',
'ts.s3.t':'3. Limitation of Liability',
'ts.s3.p':'In no event will the authors be liable for any claim, damages, or other liability arising from the use of the app. The app processes documents locally and never transmits them; your files remain under your control at all times.',
'ts.s4.t':'4. Acceptable Use',
'ts.s4.p':'You agree not to use PDflow to violate any law, infringe intellectual property, generate forged documents, or harass others. The app is a productivity tool, not a forgery kit. We may refuse support for these use cases.',
'ts.s5.t':'5. Updates',
'ts.s5.p':'We release updates to fix bugs, improve performance, and add features. Updates are optional — you may continue using any version you have installed. The app does not auto-update without your consent.',
'ts.s6.t':'6. Third-Party Components',
'ts.s6.p':'PDflow is built on PySide6 (LGPL), PyMuPDF (AGPL), RapidOCR (Apache 2.0), ReportLab, and other open-source libraries. Their licenses apply to their respective components. The full license list is available in the About section of the app.',
'ts.s7.t':'7. Termination',
'ts.s7.p':'You may stop using PDflow at any time — simply uninstall. Since there is no account and no cloud sync, nothing needs to be deleted on our end. This agreement terminates automatically when you uninstall the app.',
'ts.s8.t':'8. Governing Law',
'ts.s8.p':'This agreement is governed by the laws of the jurisdiction in which the project maintainer resides, without regard to conflict of principles. If any provision is unenforceable, the remaining provisions remain in effect.',
'ts.s9.t':'9. Contact',
'ts.s9.p':'Questions? Email support@yinliupdflow.com or open an issue on GitHub. We respond within 2 business days.',
'ts.cta.h2':'Ready to try PDflow?',
'ts.cta.p':'Free download. Local-first. No signup required.',
'ts.cta.meta':'v1.2.0 · MIT License'
},
zh:{
/* Meta */
'page.title':'PDflow — 可视化文档生成与 PDF 转换 | 印流',
'page.desc':'可视化生成文档，PDF 转换含 OCR，本地优先，专业导出。为设计师和团队打造的一站式 PDF 平台。',
'og.title':'PDflow — 可视化文档生成与 PDF 转换',
'og.desc':'可视化生成文档，PDF 转换含 OCR，本地优先，专业导出。',

/* Nav */
'nav.product':'产品',
'nav.compare':'对比',
'nav.cases':'案例',
'nav.faq':'常见问题',
'nav.blog':'博客',
'nav.download':'免费下载',
'lang.toggle':'EN',

/* Hero */
'hero.badge':'v1.2 现已发布',
'hero.h1':'像设计一样<br>创建文档',
'hero.sub':'可视化文档生成工具，集成 PDF 转换、OCR 和专业导出。无需联网，无需注册。',
'hero.cta.primary':'免费下载',
'hero.cta.secondary':'查看产品',
'hero.trust':'★★★★★ <span>永久免费 · 已生成 10,000+ 份文档</span>',
'hero.float1':'PDF → Excel',
'hero.float2':'OCR 识别',
'hero.float3':'PDF 导出',

/* S2 Demo */
's2.tag':'产品演示',
's2.h2':'PDflow 实际效果',
's2.desc':'从导入到导出，实时预览，专业输出。',
's2.tab1':'发票模板',
's2.tab2':'PDF → Excel',
's2.f1.num':'6+','s2.f1.t':'模板库','s2.f1.s':'发票、合同、报告',
's2.f2.num':'10+','s2.f2.t':'PDF 工具','s2.f2.s':'合并、拆分、压缩、OCR',
's2.f3.num':'100%','s2.f3.t':'本地处理','s2.f3.s':'无需联网，无需上传',
's2.f4.num':'<3秒','s2.f4.t':'导出速度','s2.f4.s':'高质量 PDF',

/* S3 Gallery */
's3.tag':'应用截图',
's3.h2':'为真实工作打造',
's3.desc':'专业模板排版、PDF 转换、完整工具箱 —— 一个桌面应用全搞定。',
's3.c1.t':'模板编辑器','s3.c1.s':'可视化排版，实时预览',
's3.c2.t':'PDF 转换','s3.c2.s':'PDF → Excel 含 OCR',
's3.c3.t':'PDF 工具箱','s3.c3.s':'合并、拆分、压缩、水印',

/* S4 Compare */
's4.tag':'对比',
's4.h2':'不一样的设计',
's4.desc':'看看 PDflow 与在线 PDF 工具有何不同。',
's4.th1':'功能','s4.th2':'PDflow','s4.th3':'在线 PDF 工具',
's4.r1':'100% 本地处理',
's4.r2':'无需上传文件',
's4.r3':'OCR 转换',
's4.r4':'专业模板',
's4.r5':'PDF 导出质量',
's4.r5.p1':'矢量','s4.r5.p2':'位图',
's4.r6':'免费（无订阅）',
's4.r7':'离线可用',
's4.r8':'隐私保障',
's4.note':'* 在线工具的 OCR 通常限制前 5 页免费，超出需付费。',

/* S5 Cases */
's5.tag':'真实案例',
's5.h2':'从繁琐到高效',
's5.desc':'真实团队，真实文档，真实节省时间。',
's5.c1.b':'自由职业者',
's5.c1.b1':'之前','s5.c1.b1t':'30 分钟手动排版',
's5.c1.a1':'之后','s5.c1.a1t':'5 分钟完成导出',
's5.c1.q':'"我几分钟就能生成发票。模板系统是救命稻草。"',
's5.c2.b':'会计',
's5.c2.b1':'之前','s5.c2.b1t':'手动 PDF → Excel 输入',
's5.c2.a1':'之后','s5.c2.a1t':'OCR 自动提取表格',
's5.c2.q':'"扫描件发票 99% 准确率，无需重录。"',
's5.c3.b':'运营',
's5.c3.b1':'之前','s5.c3.b1t':'每个 PDF 任务用不同工具',
's5.c3.a1':'之后','s5.c3.a1t':'一个应用搞定合并、拆分、OCR',
's5.c3.q':'"合并、压缩、OCR 全部本地处理，数据零外泄。"',
's5.c4.b':'设计公司',
's5.c4.b1':'之前','s5.c4.b1t':'客户文档需重新排版',
's5.c4.a1':'之后','s5.c4.a1t':'模板 + 品牌风格 = 搞定',
's5.c4.q':'"品牌模板格式一致，客户非常喜欢。"',

/* S6 FAQ */
's6.tag':'常见问题',
's6.h2':'高频问题<br>集中解答',
's6.search':'搜索问题...',
's6.q1':'PDflow 真的免费吗？',
's6.a1':'是的。所有功能全开放，无隐藏费用，无付费层级，无广告。为社区而生。',
's6.q2':'支持哪些平台？',
's6.a2':'Windows 10/11（64 位）。macOS 和 Linux 在路线图中。基于 PySide6 Qt 6 开发。',
's6.q3':'数据在本地处理吗？',
's6.a3':'100% 本地。所有处理都在你的电脑上完成，无云端上传，无追踪，完全离线可用。',
's6.q4':'PDF → Excel 转换原理？',
's6.a4':'PyMuPDF + RapidOCR 识别扫描文档，保留表格结构，全局对齐列宽，图片直接嵌入 Excel 单元格。',
's6.q5':'文件大小和版本？',
's6.a5':'当前版本 v1.2.0，安装约 78 MB，支持 Windows 10/11。SHA256 校验和见下载页。',
's6.q6':'可以商用吗？',
's6.a6':'完全可以。为你的业务或客户生成发票、合同、报告，无任何限制。商用免费。',

/* S7 Download */
's7.h2':'开始创建更好的<br>文档',
's7.p':'免费下载，本地优先，无需注册。',
's7.cta.primary':'免费下载',
's7.cta.github':'GitHub 源码',
's7.cta.changelog':'查看更新日志 →',
's7.i.version':'版本','s7.i.updated':'更新','s7.i.platform':'平台','s7.i.size':'大小','s7.i.sha':'SHA256',
's7.i.version.v':'1.2.0',
's7.i.updated.v':'2026 年 6 月',
's7.i.platform.v':'Windows 10/11',
's7.i.size.v':'215 MB',
's7.users':'已被 <strong>1,200+</strong> 用户下载 · 抢先体验',

/* Footer */
'f.tag':'文档创建平台。<br>生成 · 转换 · 导出 · 全部本地。',
'f.h.download':'下载',
'f.d.windows':'Windows','f.d.notes':'发布说明','f.d.changelog':'更新日志','f.d.sha':'SHA256 校验',
'f.h.product':'产品',
'f.p.excel':'PDF → Excel','f.p.tools':'PDF 工具','f.p.template':'模板编辑器','f.p.demo':'产品演示','f.p.compare':'功能对比',
'f.h.resources':'资源',
'f.r.blog':'博客','f.r.faq':'常见问题','f.r.docs':'文档','f.r.api':'API','f.r.roadmap':'路线图',
'f.h.company':'公司',
'f.c.about':'关于','f.c.privacy':'隐私政策','f.c.terms':'服务条款','f.c.support':'技术支持','f.c.github':'GitHub',
'f.bot.left':'&copy; 2026 印流PDflow. <a href="#">隐私</a> · <a href="#">条款</a> · <a href="mailto:support@yinliupdflow.com">联系</a>',
'f.bot.right':'基于 PySide6 · RapidOCR · PyMuPDF · 最近更新 2026 年 6 月',

/* Blog index */
'blog.tag':'博客',
'blog.h1':'PDF 技巧、指南<br>与产品资讯',
'blog.sub':'学习如何用好 PDflow——从 PDF 转换到模板设计，再到文档自动化。',
'blog.read':'阅读全文 →',
'blog.cta':'想看 PDflow 实际效果？',
'blog.cta.btn':'免费下载',
'blog.back':'← 返回博客',

/* Blog detail: v1.2 release */
'v12.breadcrumb':'博客 / 产品发布',
'v12.h1':'PDflow v1.2：更快、更清爽、<br>更智能的一次更新',
'v12.lede':'全新模板库，PDF 导出速度提升 50%，重新设计的首页 UI，更完善的 OCR 引擎。一切本地处理，免费且无限制。',
'v12.s1.t':'全新模板库',
'v12.s1.p':'我们从零重写了模板系统。新模板库内置 6+ 套专业设计的模板，涵盖发票、合同、报告、公告、产品规格——全部基于统一的 PDflow 排版引擎构建。每个模板都支持实时预览、品牌色配置和基于 JSON 的定制，无需重写即可调整每个细节。',
'v12.s2.t':'PDF 导出速度提升 50%',
'v12.s2.p':'导出管线围绕 PyMuPDF 的矢量原语重写。原先需要 6 秒的大型文档，现在 3 秒内即可完成——实际报告（含图片、表格、CJK 字体）上速度提升 50%。导出时的内存占用也减少了 30%，普通配置的电脑处理长文档也更流畅。',
'v12.s3.t':'首页 UI 焕新',
'v12.s3.p':'桌面首页、工具箱启动器、设置面板都完成了视觉升级。新的字号阶梯、更精细的卡片间距、重新设计的导航抽屉，让整个应用更加统一协调。主题系统通过统一的 ThemeManager 管理——不再有窗口间的样式漂移。',
'v12.s4.t':'OCR 引擎更完善',
'v12.s4.p':'RapidOCR 新增了更智能的预处理流程，支持倾斜校正、降噪和对比度归一化。扫描件发票和小字号、低分辨率文本的识别准确率提升 8–15%。CJK 多语言识别现已默认开启，无需额外下载模型。',
'v12.s5.t':'问题修复与稳定性',
'v12.s5.p':'PDF 合并现在能正确保留页面旋转标志；水印预览在缩放时不再闪烁；压缩处理混合方向页面不再崩溃；模板编辑器的撤销历史限定在 100 步，长时间操作也不会出现内存失控。',
'v12.s6.t':'升级与反馈',
'v12.s6.p':'v1.2 是免费的原地升级——直接在 v1.1 之上安装即可。设置、草稿和模板库都会保留。如遇任何问题，请在 GitHub 提 issue 或邮件联系我们。v1.3 将聚焦可视化布局画布，支持吸附辅助线和缩放控制。',
'v12.kw':'发布说明 · v1.2.0 · 2026 年 6 月 23 日 · 阅读约 5 分钟',
'v12.cta':'免费下载 v1.2',
'blog.tag.product':'产品','blog.tag.conv':'转换','blog.tag.priv':'隐私','blog.tag.tpl':'模板','blog.tag.tb':'工具箱','blog.tag.ocr':'OCR','blog.tag.wf':'工作流','blog.tag.cmp':'对比',
'blog.v12.title':'PDflow v1.2: 版本更新说明','blog.v12.meta':'2026 年 6 月 23 日 · 阅读约 5 分钟','blog.v12.desc':'全新模板库，PDF 导出速度提升 50%，重新设计的首页 UI，更完善的 OCR 引擎。一切本地处理——无云端、无注册，依然 100% 免费。',
'blog.bp.title':'2026 年最佳 PDF 转 Excel 工具','blog.bp.meta':'2026 年 6 月 12 日 · 阅读约 6 分钟','blog.bp.desc':'对比主流 PDF 转 Excel 工具，分析为什么本地 OCR 处理比云端方案更准确，以及 PDflow 如何处理扫描表格、保留图片、对齐列宽。',
'blog.ocr.title':'如何使用 OCR 将 PDF 转为 Excel','blog.ocr.meta':'2026 年 6 月 8 日 · 阅读约 5 分钟','blog.ocr.desc':'将扫描 PDF 转为可编辑 Excel 的完整步骤指南。涵盖 OCR 引擎选型、表格结构识别、嵌入图片保留技巧。',
'blog.priv.title':'为什么本地 PDF 工具关乎隐私','blog.priv.meta':'2026 年 6 月 5 日 · 阅读约 4 分钟','blog.priv.desc':'大多数在线 PDF 工具会把你的文档上传到第三方服务器。本地优先的文档处理才是数据隐私与安全的唯一保障。',
'blog.free.title':'自由职业者的文档自动化','blog.free.meta':'2026 年 6 月 1 日 · 阅读约 7 分钟','blog.free.desc':'如何用自动发票模板、合同生成、专业 PDF 导出每周省下数小时——完全无需云端订阅。',
'blog.inv.title':'如何设计专业发票模板','blog.inv.meta':'2026 年 5 月 28 日 · 阅读约 6 分钟','blog.inv.desc':'设计让客户眼前一亮的发票。涵盖 CJK 字体排版、表格布局最佳实践、品牌一致的格式。',
'blog.tb.title':'PDF 工具完整指南：合并、拆分、压缩','blog.tb.meta':'2026 年 5 月 24 日 · 阅读约 8 分钟','blog.tb.desc':'PDF 文件管理的方方面面：合并多文档、按页拆分、邮件压缩、添加水印。',
'blog.rt.title':'RapidOCR vs Tesseract：哪个 OCR 引擎更强？','blog.rt.meta':'2026 年 5 月 20 日 · 阅读约 5 分钟','blog.rt.desc':'OCR 引擎全方位对比：速度、准确率、CJK 语言支持，以及为什么 RapidOCR 是现代文档工作流的首选。',
'blog.wf.title':'如何搭建本地化文档工作流','blog.wf.meta':'2026 年 5 月 16 日 · 阅读约 6 分钟','blog.wf.desc':'从模板创建到 PDF 导出——设计一套数据本地化、流程高效、输出专业的工作流。',
'blog.p2i.title':'PDF 转图片：何时以及如何转换','blog.p2i.meta':'2026 年 5 月 12 日 · 阅读约 4 分钟','blog.p2i.desc':'PDF 转图片的应用场景。高分辨率导出用于演示、社交媒体预览、印刷级素材。',
'blog.v11.title':'PDflow v1.1: 版本更新说明（归档）','blog.v11.meta':'2026 年 5 月 8 日 · 阅读约 3 分钟','blog.v11.desc':'PDflow v1.1 发布说明。OCR 准确率提升、新增模板、导出引擎加速。已被 v1.2 替代。',
'blog.why.title':'我们为什么打造 PDflow：本地优先的文档平台','blog.why.meta':'2026 年 5 月 2 日 · 阅读约 5 分钟','blog.why.desc':'PDflow 背后的故事。为什么本地处理、隐私优先设计、专业模板在现代文档工作流中愈发重要。',
'blog.vsp.title':'PDflow vs PDFgear：哪款适合你的工作流？','blog.vsp.meta':'2026 年 4 月 28 日 · 阅读约 7 分钟','blog.vsp.desc':'PDflow 与 PDFgear 详细对比。功能、隐私、模板支持、OCR 质量，以及哪款工具更适合设计师与团队。',

/* Subpage: PDF → Excel */
'pe.tag':'PDF → Excel',
'pe.h1':'PDF 转 Excel<br>自带 OCR。',
'pe.sub':'保留表格结构，本地优先处理，扫描文档 99% 准确率。无需上传，无需注册。',
'pe.cta.primary':'免费下载',
'pe.cta.secondary':'了解更多',
'pe.f1.t':'表格结构完整保留',
'pe.f1.s':'列、行、合并单元格精确对齐，导出后可直接在 Excel 中编辑。',
'pe.f2.t':'扫描 PDF OCR 识别',
'pe.f2.s':'RapidOCR 引擎处理扫描件发票、收据、合同，准确率高达 99%。',
'pe.f3.t':'100% 本地处理',
'pe.f3.s':'文档不出本机，无云端上传，无追踪，隐私完整保护。',
'pe.cta.h2':'几秒即可将 PDF<br>转为 Excel',
'pe.cta.p':'免费下载，本地优先，无需注册。',
'pe.cta.meta':'v1.2.0 · Windows 10/11 · 215 MB',

/* Subpage: PDF Tools */
'pt.tag':'PDF 工具',
'pt.h1':'合并、拆分、压缩。<br>一个应用全搞定。',
'pt.sub':'10+ 种 PDF 工具，100% 本地处理。无需上传、无需注册，为设计师与团队输出专业级成果。',
'pt.cta.primary':'免费下载',
'pt.cta.secondary':'了解更多',
'pt.f1.t':'合并与拆分',
'pt.f1.s':'把多份 PDF 合并为一份，或按页范围拆分；支持批量操作。',
'pt.f2.t':'压缩',
'pt.f2.s':'在不损失矢量文字清晰度的前提下，PDF 体积最高缩小 80%。',
'pt.f3.t':'水印',
'pt.f3.s':'添加文字或图片水印，精确控制位置、透明度、旋转与页码。',
'pt.f4.t':'页面管理',
'pt.f4.s':'重排、旋转、删除或插入页面，导出前缩略图预览。',
'pt.f5.t':'格式转换',
'pt.f5.s':'PDF 与图片互转、PDF → Word / Excel / PPT，双向支持。',
'pt.f6.t':'隐私优先',
'pt.f6.s':'所有操作在本机完成，文档不外传，完全离线可用。',
'pt.cta.h2':'10+ 种 PDF 工具<br>一个应用，全部本地',
'pt.cta.p':'免费下载，本地优先，无需注册。',
'pt.cta.meta':'v1.2.0 · Windows 10/11 · 215 MB',

/* Subpage: Template Builder */
'tb.tag':'模板编辑器',
'tb.h1':'像设计一样<br>创建文档。',
'tb.sub':'可视化模板排版，涵盖发票、合同、报告。实时预览，一键导出 PDF，100% 本地。',
'tb.cta.primary':'免费下载',
'tb.cta.secondary':'了解更多',
'tb.f1.t':'可视化编辑器',
'tb.f1.s':'拖拽式排版，吸附网格，实时预览，无需写代码。',
'tb.f2.t':'6+ 内置模板',
'tb.f2.s':'发票、合同、报告、公告、名片、产品规格等场景全覆盖。',
'tb.f3.t':'品牌风格',
'tb.f3.s':'保存品牌色、字体与 Logo，跨模板复用，确保输出风格一致。',
'tb.f4.t':'草稿保存',
'tb.f4.s':'自动保存进度，随时重新打开继续编辑，不丢失任何修改。',
'tb.f5.t':'矢量 PDF 导出',
'tb.f5.s':'高质量矢量 PDF 输出，文字清晰、版式精准，专业交付。',
'tb.f6.t':'JSON 模板',
'tb.f6.s':'面向设计师的 JSON 格式，既可手写也可用界面编辑，方便团队共享。',
'tb.cta.h2':'今天就开始<br>创建精美文档',
'tb.cta.p':'免费下载，本地优先，无需注册。',
'tb.cta.meta':'v1.2.0 · Windows 10/11 · 215 MB',

/* Subpage: About */
'ab.tag':'关于',
'ab.h1':'为设计师而生，<br>由本地优先驱动。',
'ab.sub':'PDflow 是免费、开源的文档平台，尊重你的隐私。所有操作在本机完成——无云端、无追踪、无注册。',
'ab.cta.primary':'免费下载',
'ab.cta.secondary':'GitHub 仓库',
'ab.c1.t':'我们的使命',
'ab.c1.p':'大多数 PDF 工具臃肿、收费，还会把你的敏感文档上传到云端。PDflow 要解决的就是这个。本地优先意味着数据从不出本机——你甚至不需要联网就能用。',
'ab.c2.t':'技术栈',
'ab.c2.p':'基于开源基础组件构建的现代技术栈。快速、私密，为追求输出质量的设计师与团队量身打造。',
'ab.c3.t':'开源',
'ab.c3.p':'PDflow 以 MIT 协议开源。前往 GitHub 查看源码、提 issue 或贡献代码，下一步做什么由社区决定。',
'ab.c4.t':'路线图',
'ab.c4.p':'v1.2（当前）— 6+ 模板与 OCR 调优。v1.3 — 可视化布局画布，支持吸附辅助线。v2.0 — PDflow Studio：统一的创作、排版与 AI 辅助导出。',
'ab.cta.h2':'现在就试 PDflow',
'ab.cta.p':'免费下载，本地优先，无需注册。',
'ab.cta.meta':'v1.2.0 · Windows 10/11 · 215 MB · MIT 协议',

/* Subpage: Privacy Policy */
'pv.tag':'隐私政策',
'pv.h1':'文档始终<br>留在本机。',
'pv.sub':'PDflow 本地优先，你的文件从不接触我们的服务器——因为我们根本没有服务器。以下是完整隐私政策，用大白话写。',
'pv.updated':'最后更新：2026 年 6 月 23 日',
'pv.s1.t':'1. 我们不收集什么',
'pv.s1.p':'PDflow 完全运行在你的电脑上。我们不运营云服务器，不收集使用数据，不记录你的文件，也不要求注册账号。App 唯一存储的数据，是你主动保存到本地磁盘的内容。',
'pv.s2.t':'2. 网站分析',
'pv.s2.p':'本网站（yinliupdflow.com）使用 Umami Cloud——一个注重隐私的分析服务。Umami 记录匿名的页面浏览、按钮点击、聚合的导航模式。它不会跨站追踪你，不使用广告 cookie，也不存储 IP 地址。数据仅用于了解网站的哪些部分有用。',
'pv.s3.t':'3. 下载链接',
'pv.s3.p':'当你下载 PDflow 时，浏览器可能将标准的 HTTP referrer 和 user-agent 发送给 GitHub 的 release 服务器。我们不会收到这些信息。',
'pv.s4.t':'4. 更新与崩溃报告',
'pv.s4.p':'App 可以通过 HTTPS 检查更新。它只向 GitHub Releases 发送当前版本号和操作系统；不发送其他任何信息。我们不会自动收集崩溃报告、错误日志或堆栈跟踪。如遇 bug，请通过 GitHub issue 手动报告。',
'pv.s5.t':'5. 你的权利',
'pv.s5.p':'因为我们不收集你的数据，所以也不存在访问、导出、删除的场景。如果通过邮件联系我们，我们会在你要求删除前一直保留，且绝不出售或分享给第三方。',
'pv.s6.t':'6. 儿童',
'pv.s6.p':'PDflow 不针对 13 岁以下儿童。我们不会明知而收集任何儿童信息。App 是工具而非服务——无注册、无档案。',
'pv.s7.t':'7. 政策变更',
'pv.s7.p':'产品演进时可能更新本政策。重要变更会在博客公告，并标注新的"最后更新"日期。变更后继续使用视为接受。',
'pv.s8.t':'8. 联系方式',
'pv.s8.p':'如有疑问，请发邮件至 support@yinliupdflow.com 或在 GitHub 提 issue。我们会在 2 个工作日内回复。',
'pv.cta.h2':'隐私应当是默认配置。',
'pv.cta.p':'可阅读我们的架构笔记，或直接下载并自行审查 App。',
'pv.cta.meta':'v1.2.0 · 本地优先 · MIT 协议',

/* Subpage: Terms of Service */
'ts.tag':'服务条款',
'ts.h1':'自由使用 PDflow。<br>无附加条件。',
'ts.sub':'PDflow 免费、开源，按"现状"提供。使用即代表同意以下条款——大白话书写，无法律黑话。',
'ts.updated':'最后更新：2026 年 6 月 23 日',
'ts.s1.t':'1. 许可证',
'ts.s1.p':'PDflow 以 MIT 协议发布。你可将其用于个人、商业或教育目的；可修改、再分发、销售基于它构建的产品。唯一要求是：版权声明与许可文本随再分发一起保留。',
'ts.s2.t':'2. 无担保',
'ts.s2.p':'PDflow 按"现状"提供，不附带任何形式的担保。我们在 Windows 10/11 上做了充分测试，但无法保证在每种硬件配置、每种语言、每种文档工作流中都完美运行。请自行判断使用。',
'ts.s3.t':'3. 责任限制',
'ts.s3.p':'在任何情况下，作者都不对因使用 App 而引起的任何索赔、损害或其他责任负责。App 在本地处理文档，从不传输它们；你的文件始终由你掌控。',
'ts.s4.t':'4. 可接受使用',
'ts.s4.p':'你同意不将 PDflow 用于违法、侵犯知识产权、伪造文件或骚扰他人。App 是生产力工具，不是伪造套件。我们可能拒绝为这些场景提供支持。',
'ts.s5.t':'5. 更新',
'ts.s5.p':'我们会发布更新来修复 Bug、提升性能、增加功能。更新是可选的——你可以继续使用已安装的任何版本。未经你同意，App 不会自动更新。',
'ts.s6.t':'6. 第三方组件',
'ts.s6.p':'PDflow 基于 PySide6（LGPL）、PyMuPDF（AGPL）、RapidOCR（Apache 2.0）、ReportLab 等开源库构建。各组件许可协议适用于其各自组件。完整许可清单见 App 的"关于"页。',
'ts.s7.t':'7. 终止',
'ts.s7.p':'你可随时停止使用 PDflow——直接卸载即可。因为没有账号和云同步，我们端无需删除任何数据。你卸载 App 时，本协议自动终止。',
'ts.s8.t':'8. 适用法律',
'ts.s8.p':'本协议受项目维护者所在司法辖区法律管辖，不考虑法律冲突原则。如某条款不可执行，其余条款继续有效。',
'ts.s9.t':'9. 联系方式',
'ts.s9.p':'如有疑问，请发邮件至 support@yinliupdflow.com 或在 GitHub 提 issue。我们会在 2 个工作日内回复。',
'ts.cta.h2':'准备好试试 PDflow 了？',
'ts.cta.p':'免费下载，本地优先，无需注册。',
'ts.cta.meta':'v1.2.0 · MIT 协议',
}
};

var KEY='pdflow_lang';
var current='en';

function getLang(){
try{
var urlLang=new URLSearchParams(window.location.search).get('lang');
if(urlLang&&DICTS[urlLang]){localStorage.setItem(KEY,urlLang);return urlLang;}
var stored=localStorage.getItem(KEY);
if(stored&&DICTS[stored])return stored;
}catch(e){}
var nav=(navigator.language||'en').toLowerCase();
return nav.indexOf('zh')===0?'zh':'en';
}

function apply(lang){
if(!DICTS[lang])return;
current=lang;
try{localStorage.setItem(KEY,lang);}catch(e){}
/* Update all data-i18n nodes */
document.querySelectorAll('[data-i18n]').forEach(function(el){
var key=el.getAttribute('data-i18n');
var val=DICTS[lang][key];
if(val!==undefined){
var attr=el.getAttribute('data-i18n-attr');
if(attr){
/* Update specific attribute (e.g. placeholder) */
el.setAttribute(attr,val);
}else if(val.indexOf('<')!==-1){
/* Support innerHTML for keys with <br> or <span> */
el.innerHTML=val;
}else{
el.textContent=val;
}
}
});
/* Update lang attribute */
document.documentElement.setAttribute('lang',lang==='zh'?'zh-CN':'en');
/* Update meta tags */
var titleEl=document.querySelector('title[data-i18n]');
if(titleEl)titleEl.textContent=DICTS[lang]['page.title'];
var descEl=document.querySelector('meta[name="description"][data-i18n]');
if(descEl)descEl.setAttribute('content',DICTS[lang]['page.desc']);
var ogTitle=document.querySelector('meta[property="og:title"][data-i18n]');
if(ogTitle)ogTitle.setAttribute('content',DICTS[lang]['og.title']);
var ogDesc=document.querySelector('meta[property="og:description"][data-i18n]');
if(ogDesc)ogDesc.setAttribute('content',DICTS[lang]['og.desc']);
var ogLocale=document.querySelector('meta[property="og:locale"]');
if(ogLocale)ogLocale.setAttribute('content',lang==='zh'?'zh_CN':'en_US');
/* Update toggle button text */
var toggle=document.getElementById('langToggle');
if(toggle){
var span=toggle.querySelector('span[data-i18n="lang.toggle"]');
if(span)span.textContent=DICTS[lang]['lang.toggle'];
toggle.setAttribute('data-lang',lang);
}
}

function toggle(){
apply(current==='en'?'zh':'en');
}

function init(){
var initial=getLang();
apply(initial);
var btn=document.getElementById('langToggle');
if(btn)btn.addEventListener('click',toggle);
}

/* Expose for interaction.js to re-apply on DOM changes */
window.PDFlowI18n={apply:apply,toggle:toggle,get current(){return current;}};

if(document.readyState==='loading'){
document.addEventListener('DOMContentLoaded',init);
}else{
init();
}

})();
